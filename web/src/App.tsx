...
export default function App() {
  ...
  const resetQuestion = () => { setAnswer(''); setResult(null); setRevealed(false); window.speechSynthesis?.cancel() }
  ...
  const speak = () => { if (!line || !('speechSynthesis' in window)) return setTtsError('当前浏览器不支持语音合成。')
    setTtsError(''); speechSynthesis.cancel(); const u = new SpeechSynthesisUtterance(line.en); u.lang = 'en-US'; u.rate = rate
    ...
    <main>
      ...
      {error ? <div class="state-card"><p>{error}</p><button onClick={fetchData}>重新加载</button></div> : !data ? <div class="state-card"><span class="loader"/>正在载入冥界档案…</div> : <>
        ...
        {!line ? <div class="state-card">没有找到匹配的台词。</div> : <section class="study-card">
          ...
          <div class="voice-row"><button class="play" onClick={speak} aria-label="播放英文台词"><span>▶</span> {mode==='dictation'?'播放题目':'朗读英文'}</button><span class="tts-note">浏览器英文语音 · 非游戏原声</span></div>{ttsError&&<p class="error">{ttsError}</p>}
          {mode === 'browse' ? <div class="dialogue"><p class="english">{line.en}</p><div class="divider"/><p class="chinese">{line.zh}</p></div> : <div class="dictation"><p class="hint-label">中文提示</p><p class="chinese">{line.zh}</p>{revealed && !result && <div class="answer-reveal"><small>答案</small>{line.en}</div>}
